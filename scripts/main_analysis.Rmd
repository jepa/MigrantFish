...
ggplot() +
  geom_sf(data = sf_fao, aes(), fill ="gray70" ) +
  geom_sf(data = sf_meow %>% group_by(realm) %>% tally(), aes(fill = realm)) +
  geom_sf(data = sf_land, aes(), fill = "black", col = "black") +
  geom_sf_label(data = sf_fao, aes(label = f_code), size = 5) +
  labs(x = "",
       y = "")

ggsave(filename = "./results/supplements/supp_1.png",
       plot = last_plot(),
       width = 10,
       height = 5
       )
...
GetResults=function(Spp,type = "strad"){
  ...
  if(type == "strad"){
    ...
    Distpath <- paste(my_path("R","Per_change_realm/"),Spp,"_perchg.csv",sep="")
  }
    ...
    if(file.exists(file_to_read)){
      ...
      for(i in 1:length(fao_areas)){
        ...
        spp_data_area <-
          ...
          left_join(fao_area_grid) %>% 
          mutate(name = ifelse(is.na(eco_code),fao_area,realm)#, # For adding stradling
                 # high_mig_spa = ifelse(is.na(eez_id),"eezs",fao_area) # for adding high migra
          ) %>%
          # View()
          # When straddling use realm when highly migratory just fao region
          # {if(is.na(category)) group_by(.,year,name) else group_by(., year,high_mig_spa)} %>%
          group_by(year,name) %>% 
        ...
        prop_chng <- spp_proportion %>% 
          ...
          group_by(taxon_key,name,period) %>% 
          ...
          # remove stuff
          mutate(keep = ifelse(name == fao_area, "n","y")) %>% 
          filter(keep == "y") %>% 
          select(-keep)
...
spp_complete <- list.files(my_path("R","Per_change_realm/"))
stradling_spp <- str_replace(spp_complete,"\\_.*","")
...
fao_meow <- meow_dbem_grid %>% 
  left_join(fao_area_grid) %>% 
  group_by(realm,fao_name = fao_area_name) %>% 
...
ggplot() + 
  geom_sf(data = sf_fao %>%  left_join(strad_fao), aes(fill = n)) +
  geom_sf(data = sf_meow_map %>% left_join(strad_meow), aes(fill = n)) +
  geom_sf(data = sf_land, aes(), fill = "grey30", color = "grey30") +
  scale_fill_viridis_c("Straddling spp.\n(n)", na.value = "white") +
  theme_bw() +
  theme(legend.title.align = 0.5) 

# Save plot 
ggsave("./results/figures/straddling_id/straddling_species.png",
       last_plot(),
       height = 6,
       width = 12
)
...
#### Straddling catch and revenue

##### Data from SAU


```{r}
sau_catch_data <- read.csv("/Volumes/Enterprise/Data/SAU/SAUCatch_FE_EEZ_FAO_taxa_sector_29May2020.csv")

sau_eez_fes <- read.csv("/Volumes/Enterprise/Data/Spatial/SAU/sau_eez_fes.csv") %>% 
  group_by(fishing_entity_id,fishing_entity) %>% 
  tally() %>% 
  select(-n)


# Estimate country-level catch
country_catch <- sau_catch_data %>% 
  filter(year > 2010,
         eezid == 0,
         taxon_key %in% shared_data$taxon_key) %>% 
  # pull(taxon_key) %>% 
  # unique() %>% 
  # length()
  # Add sector types
  group_by(year,fishing_entity_id,fao_area_id,taxon_key) %>% 
  summarise(sumcatch= sum(sumcatch)
  ) %>% 
  # average of 10 years
  group_by(fishing_entity_id,fao_area_id,taxon_key) %>% 
  summarise(mean_catch = mean(sumcatch)) %>% 
  left_join(sau_eez_fes)  %>% 
  # Get country totals
  group_by(fishing_entity_id,fishing_entity) %>% 
  summarise(total_catch = sum(mean_catch),
            n_stocks = length(unique(taxon_key))
  ) %>% 
  arrange(desc(total_catch))

# Estimate FAO zone catch
# fao_catch <- 
sau_catch_data %>% 
  filter(year > 2010,
         eezid == 0,
...
shared_data %>% 
  ...
  summarise(value = length(unique(territory))) %>%   # number of eezs sharing the stock
  top_n(5,value) %>% 
  arrange(desc(value))
```
...
Species_Data <- shared_data %>% 
  left_join(meow_dbem_eez) %>% 
  group_by(common_name,group = realm) %>% 
  summarise(value = length(unique(territory))) %>%   # number of eezs sharing the stock
  group_by(group) %>%
  top_n(10,value) # select the top 50% shared species
...
Plot_Species_Data <- Species_Data %>% 
  left_join(high_mig_spp) %>% 
  mutate(category = ifelse(is.na(cat),"straddling","high migr."))
...
ggplot(Species_Data, 
  ...
  geom_segment(data=grid_data, 
               aes(x = end, y = 65, xend = start, yend = 65), 
               colour = Ticks_Color, alpha=1, size=0.3, inherit.aes = FALSE) +
  geom_segment(data=grid_data,
               aes(x = end, y = 45, xend = start, yend = 45),  
               colour = Ticks_Color, alpha=1, size=0.3, inherit.aes = FALSE) +
  geom_segment(data=grid_data, 
  ...
  #              size=0.6 , 
  #              inherit.aes = FALSE) +
  ylim(-50,105) + # sets the circle the first value the central the last value how wide
  theme_bw() +
  theme(
...
# Save plot
ggsave(filename = "./results/figures/species_ohi.jpg",
       plot = last_plot(),
       width = 12,
       height = 12,
       units = "in")
...
### Non-migratpry straddling species circle

#### Numbers for paper
```{r}

# Numbers for paper
# Non Annex I species level
shared_data %>% 
  filter(!taxon_name %in% h_migratory_spp_id$scientific_name) %>% 
  left_join(meow_dbem_eez) %>% 
  group_by(common_name) %>% 
  summarise(value = length(unique(territory))) %>%   # number of eezs sharing the stock
  arrange(desc(value))
...
shared_data %>% 
  filter(!taxon_name %in% h_migratory_spp$scientific_name) %>% 
  left_join(meow_dbem_eez) %>% 
  group_by(common_name) %>% 
  summarise(value = length(unique(territory))) %>%   # number of eezs sharing the stock
  arrange(desc(value))
...
plot_data <- prop_data %>% 
  group_by(name,fao_area,esm,rcp,change) %>% 
  tally() %>% 
  group_by(name,fao_area,rcp,change) %>% 
...
fao_centroids <-
  as.data.frame(st_centroid(sf_fao)) %>% 
  ungroup() %>% 
  select(fao_area = fao_name,f_code, the_geom) %>% 
  separate(col = the_geom, into = c("lon", "lat"), sep = "\\,")

# Remove "c()"
fao_centroids$lon <- gsub("\\(","",fao_centroids$lon)
fao_centroids$lon <- gsub("c","",fao_centroids$lon)
fao_centroids$lat <- gsub(")","",fao_centroids$lat)

# Modify annolying points (most of them!)
fao_centroids <- fao_centroids %>% 
  mutate(
    # modify latitudes
    lat = ifelse(fao_area == "Pacific, Northeast",50,
                 ifelse(fao_area == "Pacific, Antarctic",-65,
                 ifelse(fao_area == "Atlantic, Northwest",50,
                        ifelse(fao_area == "Atlantic, Antarctic",-65,
                               ifelse(fao_area == "Arctic Sea",85,
                                      ifelse(fao_area == "Indian Ocean, Western",-35,
                                             ifelse(fao_area == "Indian Ocean, Eastern",-45,
                                                    ifelse(fao_area == "Indian Ocean, Antarctic",-62,lat)
                                             ))))))),
    # modify longitudes
    lon = ifelse(fao_area == "Pacific, Southwest",-135,
                 ifelse(fao_area == "Pacific, Southeast",-105,
                        ifelse(fao_area == "Pacific, Antarctic",-135,
                        ifelse(fao_area == "Pacific, Northwest",160,
                               ifelse(fao_area == "Pacific, Western Central",170,
                                      ifelse(fao_area == "Atlantic, Northwest",-45,
                                             ifelse(fao_area == "Atlantic, Northeast",-20,
                                                    ifelse(fao_area == "Atlantic, Western Central",-50,
                                                           ifelse(fao_area == "Atlantic, Southwest",-30,
                                                                  ifelse(fao_area == "Atlantic, Western Central",-50,
                                                                         ifelse(fao_area == "Atlantic, Eastern Central",-30,
                                                                                ifelse(fao_area == "Arctic Sea",-120,
                                                                                       lon
                                                                                ))))))))))))
  )
...
# ggplot() +
#   geom_sf(data = sf_fao, aes(), fill ="white" ) +
#   geom_sf(data = sf_land, aes(), fill = "gray30", col = "gray30") +
#   geom_text(data = fao_centroids, aes(x = as.numeric(lon), y = as.numeric(lat), label = fao_area)) +
...
meow_centroids <- meow_centroids %>% 
  mutate(
    lat = ifelse(name == "Temperate Northern Atlantic",72,
               ifelse(name == "Western Indo-Pacific",-9,
                      ifelse(name == "Eastern Indo-Pacific",-4,
                             ifelse(name == "Southern Ocean",-65,
                                    ifelse(name == "Temperate South America",-55,lat
               ))))),
    lon = ifelse(name == "Arctic",-55,
                 ifelse(name == "Temperate Northern Pacific",-130,
                 ifelse(name == "Tropical Eastern Pacific",-90,
                        ifelse(name == "Eastern Indo-Pacific",95,
                               ifelse(name == "Temperate South America",-60,
                                      ifelse(name == "Tropical Atlantic",-55,
                                             ifelse(name == "Temperate North",42,
                                                    ifelse(name == "Temperate Northern Atlantic",42,
                                                           ifelse(name == "Western Indo-Pacific",47,lon
                        )))))))))
  )
...
Data <-
  ...
  left_join(fao_centroids,
            by = "fao_area") %>%
  rename(target = fao_area,
         end_lon = lon,
         end_lat = lat) %>% 
  ungroup() %>% 
  drop_na() %>% 
...
ggplot() +
  ...
  geom_sf(data = sf_land, aes(), fill = "gray30", col = "gray30") +
  geom_point(data = fao_centroids, aes(x = as.numeric(lon), y = as.numeric(lat))) +
  geom_text(data = fao_centroids, aes(x = as.numeric(lon), y = as.numeric(lat), label = fao_area)) +
  geom_point(data = meow_centroids, aes(x = as.numeric(lon), y = as.numeric(lat))) +
  geom_text(data = meow_centroids, aes(x = as.numeric(lon), y = as.numeric(lat), label = name), color = "blue") +
...
nodes <- data.frame(name = c(sankey_df %>% ungroup() %>% select(node = name) %>% pull(node) %>% unique(), sankey_df %>% ungroup() %>% select(node = fao_area)%>% pull(node) %>% unique()))


# get ids (identify each region by number)
ids <- nodes %>% 
  rowid_to_column("id") %>% 
  rename(fao_area = name) %>% 
  bind_cols(nodes) %>% 
  mutate(
    region = ifelse(name %in% sankey_df$name,"name","fao_area")
  ) %>% 
...
mycolor_g <- c(viridis(12, alpha = 1, begin = 0, end = 1, option = "D"),
             viridis(27-12, alpha = 1, begin = 0, end = 1, option = "F")
             )
# Color scale for loses
mycolor_l <- c(viridis(11, alpha = 1, begin = 0, end = 1, option = "D"),
             viridis(24-11, alpha = 1, begin = 0, end = 1, option = "F")
             )

# Color scale for no change
mycolor_s <- c(viridis(12, alpha = 1, begin = 0, end = 1, option = "D"),
             viridis(27-12, alpha = 1, begin = 0, end = 1, option = "F")
             )
